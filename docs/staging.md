...
4. `pnpm start:hosted` uses Render's assigned HTTPS `RENDER_EXTERNAL_URL` as `APP_ORIGIN` when `RENDER=true`. A custom domain requires an explicit exact HTTPS `APP_ORIGIN` without a path. Set `STAGING_MODE=true`; let Render generate secret `STAGING_INVITE_CODE` (24–128 characters). Optional `STAGING_INVITE_CODES` adds comma-separated codes, each 24–128 characters without surrounding spaces. Either setting can supply the invitations; both are accepted together. Codes are reusable, have no automatic expiry and authorize signup only. Remove a code from the hosting environment and redeploy to revoke it; existing accounts can still log in. Never store actual codes in Git. Pin `NODE_VERSION=24`. Never set `ALLOW_LOCAL_DB`, `DATA_DIR`, `TEST_DATABASE_URL`, `DEV_AUTH_BYPASS` or development credentials on the host.
