...
@njit(cache=True)
def worlds(seed):
    """1326*1225 legal deals, one sampled board each, with suit-symmetry closure.

    Each row stands for all 24 globally renamed physical worlds at 1/24 weight.
    Every stored observation/outcome is invariant under those renamings, so the
    closure is represented exactly without repeated rows. The complete physical
    hole prior remains uniform; root suit classes are then lossless symmetries.
    """
    np.random.seed(seed)
    n = 1326*1225
...
def train(data, actor, street, children, count, offsets, payoff, size,
          iterations, batch, seed, algorithm, root, oracle, locked, full_batch=False):
    np.random.seed(seed)
    ...
    for iteration in range(1,iterations+1):
      ...
      for sample in range(batch):
        w=sample if full_batch else np.random.randint(len(data))
