...
export async function verifiedNode(db:Database){
  let ready=initialized.get(db);
  if(!ready){ready=publishVerifiedSolution(db,packagedArtifact).then(r=>{if(r.status!=='VERIFIED')throw new Error('Packaged artifact failed independent verification.');}).catch(e=>{initialized.delete(db);throw e;});initialized.set(db,ready);}
  await ready;
  return new VerifiedStrategyProvider(db).getNode({context:riverContext(),treeSha256:RIVER_TREE_SHA});
}
...
export async function verifiedProgress(db:Database,userId:string,now=new Date()):Promise<VerifiedProgress>{
  const a=await verifiedNode(db);
...
export async function startVerifiedQuestion(db:Database,userId:string,mode:VerifiedMode,selection:VerifiedSelection):Promise<VerifiedQuestion>{
  const [a,progress]=await Promise.all([verifiedNode(db),verifiedProgress(db,userId)]);
...
export async function answerVerifiedQuestion(db:Database,userId:string,input:{id:string;action?:string;frequencies?:Record<string,number>}):Promise<VerifiedQuestion>{
...
export const verifiedStudyGet=route(async(request,db)=>{
  ...
  const [a,progress]=await Promise.all([verifiedNode(db),verifiedProgress(db,user.id)]);
...
export const verifiedTrainingPost=route(async(request,db)=>{
  const user=await requireUser(db,readSessionToken(request));await enforceRateLimit(db,`verified-training:${user.id}`,240,900);
  ...
  const question=input.operation==='start'?await startVerifiedQuestion(db,user.id,input.mode,input.selection):await answerVerifiedQuestion(db,user.id,input);
  const a=await verifiedNode(db);let state=replayPublicHistory(a.context);
  if(question.action && question.feedback){const action=a.actions.find(x=>x.id===question.action);if(action)state=applyPublicAction(state,action.type==='RAISE'?{actor:a.context.hero,type:'RAISE',toBb:action.toBb}:{actor:a.context.hero,type:action.type});}
  return {question,progress:await verifiedProgress(db,user.id),state};
},{mutation:true});
