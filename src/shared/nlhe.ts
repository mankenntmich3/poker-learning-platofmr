...
export interface NlheConfig { game: 'nlhe'; format: '6max-cash'; stackBb: number; hero: Position; villain: Position | null; scenario: NlheScenario; openBb?: number; threeBetBb?: number; fourBetBb?: number }
...
export interface NlheProgress {
  ...
  sessions: { id: string; label: string; answered: number; complete: boolean; study?: boolean }[];
}
...
export function validNlheConfig(config: NlheConfig): boolean {
  if ([config.openBb, config.threeBetBb, config.fourBetBb].some(size => size !== undefined && (!Number.isFinite(size) || size < 2 || size > config.stackBb))) return false;
...
export function configSearch(config: NlheConfig): string {
  return new URLSearchParams({ stack: String(config.stackBb), hero: config.hero, scenario: config.scenario, ...(config.villain ? { villain: config.villain } : {}), ...(config.openBb !== undefined ? {open:String(config.openBb)}:{}), ...(config.threeBetBb !== undefined ? {threeBet:String(config.threeBetBb)}:{}), ...(config.fourBetBb !== undefined ? {fourBet:String(config.fourBetBb)}:{}) }).toString();
}
export function configFromSearch(search: URLSearchParams): NlheConfig {
  ...
  const config: NlheConfig = { ...DEFAULT_NLHE, stackBb: Number(search.get('stack') || 100), hero: (search.get('hero') || 'BTN') as Position,
    ...
    ...(search.has('open') ? {openBb:Number(search.get('open'))}:{}), ...(search.has('threeBet') ? {threeBetBb:Number(search.get('threeBet'))}:{}), ...(search.has('fourBet') ? {fourBetBb:Number(search.get('fourBet'))}:{}) };
