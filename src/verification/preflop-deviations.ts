...
import { applyPublicEvent, replayPublicHistory, type TournamentState } from '@/domain/tournament-state';
...
export type DiagnosticPolicyMapping = (state: TournamentState, hole: Combo, history: PublicEvent[], legalIds: string[]) => Record<string, number>;
export function measurePreflopDeviations(input: FrozenPreflopCandidate, seed: string, samples: number, failureProbability = 1e-9, runtimeLimitMs = 300000, mapping?: DiagnosticPolicyMapping) {
  ...
  const run = (holes: Combo[], board: Card[], draws: number[], deviation: Deviation | null, cache: Map<string, { hole: Combo; deals: Card[][] }>): number[] => {
    ...
    for (let depth = 0; depth < 128; depth++) {
      ...
      if (!chosen) {
        ...
        if (!row && !mapping) defaultLookups++;
        const policy = mapping ? mapping(state, holes[actor], history, edges.map(e => e.id)) : row?.actions ?? Object.fromEntries(edges.map(e => [e.id, 1 / edges.length]));
