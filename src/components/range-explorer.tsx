...
import { preflopStudyUrl } from '@/shared/study-link';
...
function RangeContent() {
  ...
  return <><PageHeader title="NLHE Preflop-Ranges" description="No-Limit Texas Hold’em · 6-max Cash" />
    ...
    {resource.loading ? <LoadingPanel label="Genau diese NLHE-Range wird geladen…" /> : resource.error ? <ErrorNotice message={resource.error} retry={resource.reload} /> : node && hand ? <><RangeProvenance provenance={node.provenance} /><div className="nlhe-context"><h2>{spotLabel(node.config)}</h2><p>{node.context}</p><p>Pot {number(node.potBb, 1)} BB · bereits investiert {number(node.investedBb, 1)} BB · Rake 0</p></div>
      ...
      <section className="nlhe-related"><h2>Postflop fortsetzen</h2><ActionLink href={preflopStudyUrl(node.config,combos[0])}>Mit {selected} zum Flop</ActionLink><p>{config.scenario==='rfi'?'Annahme: BB callt dein Open.':config.scenario==='vs-open'?'Fortsetzung: Hero callt das Open.':'Fortsetzung: Hero callt die 3-Bet.'} Alle anderen Spieler folden. Stack, Positionen, Größen und Hand werden übernommen.</p><h2>Verwandte Spots</h2><div>{node.related.map(next => <Link className="button button-secondary" key={configSearch(next)} href={`/ranges?${configSearch(next)}`}>{spotLabel(next)}</Link>)}</div></section></> : null}</>;
}
