...
function RangeContent() {
  ...
  function choose(next: NlheConfig) {
    setInputError('');
    ...
    router.replace(`/ranges?${configSearch(next)}`, { scroll: false });
  }
  ...
  return <><PageHeader title="NLHE Preflop-Ranges" description="No-Limit Texas Hold’em · 6-max Cash" />
    <section className="nlhe-controls panel" aria-label="Preflop-Spot wählen"><label>Effektiver Stack<select aria-label="Effektiver Stack" value={STACK_DEPTHS.some(n => n === config.stackBb) ? config.stackBb : 'custom'} onChange={event => { if (event.target.value === 'custom') setCustom(true); else choose({ ...config, stackBb: Number(event.target.value) }); }}>{STACK_DEPTHS.map(stack => <option key={stack} value={stack}>{stack} BB</option>)}<option value="custom">Eigener Stack</option></select></label>
      <label>Deine Position<select aria-label="Deine Position" value={config.hero} onChange={event => choose({ ...config, hero: event.target.value as Position })}>{SIX_MAX_POSITIONS.map(position => <option key={position}>{position}</option>)}</select></label>
      <label>Vorgeschichte<select aria-label="Vorgeschichte" value={config.scenario} onChange={event => choose({ ...config, scenario: event.target.value as NlheScenario })}><option value="rfi" disabled={config.hero === 'BB'}>{SCENARIO_LABELS.rfi}</option><option value="vs-open" disabled={config.hero === 'UTG'}>{SCENARIO_LABELS['vs-open']}</option><option value="vs-3bet" disabled={config.hero === 'BB'}>{SCENARIO_LABELS['vs-3bet']}</option></select></label>
      {config.scenario !== 'rfi' ? <label>Gegnerposition<select aria-label="Gegnerposition" value={config.villain || ''} onChange={event => choose({ ...config, villain: event.target.value as Position })}>{possibleVillains(config).map(position => <option key={position}>{position}</option>)}</select></label> : <div className="nlhe-controls-note">RFI = Raise First In.<br />BB kann einen ungeöffneten Pot nicht mehr eröffnen.</div>}
      {custom ? <form className="custom-stack" onSubmit={event => { event.preventDefault(); const value = Number(new FormData(event.currentTarget).get('stack')); if (Number.isFinite(value)) choose({ ...config, stackBb: value }); }}><label>Eigener Stack in BB<input name="stack" type="number" min="10" max="500" step="0.01" defaultValue={config.stackBb} required /></label><button className="button button-secondary">Stack anwenden</button></form> : null}</section>
